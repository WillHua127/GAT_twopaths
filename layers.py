...
class GraphAttentionLayer(nn.Module):
    ...
    def forward(self, input, adj):
        ...
        h_high_alike = torch.zeros_like(h_high)
        h_low_alike = torch.zeros_like(h_low)
        ...
        assert not torch.isnan(edge_e_low).any()
        # edge_e: E
        ...
        count = 0
        for i in edge[0, :]:
            h_high_alike[i] = torch.add(h_high_alike[i],h_high[edge[0, :], :][count])
            h_low_alike[i] = torch.add(h_low_alike[i], h_low[edge[0, :], :][count])
            count += count
            
        h_prime_high = self.special_spmm(edge, edge_e_high, torch.Size([N, N]), h_high_alike)
        h_prime_low = self.special_spmm(edge, edge_e_low, torch.Size([N, N]), h_low_alike)
        assert not torch.isnan(h_prime_high).any()
        assert not torch.isnan(h_prime_low).any()
        # h_prime: N x out
